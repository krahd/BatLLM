    ...
    def load_from_config(self, cfg: Dict[str, Any]):
        ...
        for key in sorted(flat):
            self.grid.add_widget(
                Label(text=f"{key}", size_hint_y=None, height=50, font_size=FONT_SMALL))
            v = flat[key]
...
class BatLLMConfigApp(App):
    ...
    def build(self):
        root = BoxLayout(orientation='vertical', padding=PAD_X, spacing=PAD_Y)

        banner = Label(
            text='BatLLM Configurator (use with care and make backups!)',
            color=(.4, 1, .4, 1),
            size_hint_x=None, size_hint_y=None, width=950, height=40,
            font_size=FONT_BASE, bold=True, halign='left', valign='middle')

        root.background_color = (0.8, 0.9, 0.8, 1)
        ...
        self.tabs = TabbedPanel(do_default_tab=False, tab_height=70,
                                tab_width=600)  # taller for bigger font
        ...
        # Resize tab headers after they exist
        Clock.schedule_once(self._resize_tabs, 0.05)
        return root


    def _resize_tabs(self, *_):
        """Dynamically size tab headers so full text is visible."""
        if not hasattr(self, 'tabs') or not self.tabs.tab_list:
            return
        for btn in self.tabs.tab_list:  # Each is a ToggleButton
            try:
                btn.font_size = FONT_SMALL  # increase font
                btn.text_size = (None, None)
                btn.texture_update()
                req_w = btn.texture_size[0] + 50  # padding
                btn.size_hint_x = None
                btn.width = max(req_w, 140)
                btn.background_color = (0.8, 0.9, 0.8, 1)  # light gray
            except Exception:
                pass
    ...
    def _add_tab(self, tabs: TabbedPanel, name: str, widget):
        ti = TabbedPanelItem(text=name, font_size=FONT_SMALL)
