...
class OtherSettingsPanel(ScrollView):
    def __init__(self, **kw):
        super().__init__(**kw)
        self.bar_width = 12
        self.grid = GridLayout(cols=2, size_hint_y=None, padding=16, spacing=16)
    ...
    def load_from_config(self, cfg: Dict[str, Any]):
        ...
        for key in sorted(flat):
            self.grid.add_widget(Label(text=key, size_hint_y=None, height=50, font_size=FONT_SMALL))
            v = flat[key]
            ...
            ti = TextInput(text=txt, multiline=False, size_hint_y=None,
                           height=50, font_size=FONT_SMALL)
            self.inputs[key] = ti
...
class LLMPanel(ScrollView):
    def __init__(self, **kw):
        ...
        for k, _ in LLM_FIELDS:
            self.grid.add_widget(Label(text=k, size_hint_y=None, height=50, font_size=FONT_SMALL))
    ...
    def refresh(self):
        def work():
            try:
                ...
                Clock.schedule_once(lambda *_: info_popup('Error', f'List failed: {e}'))
    ...
    def run(self):
        ...
        def work():
            try:
                ...
                Clock.schedule_once(lambda *_: info_popup('Error', f'Run failed: {e}'))
        threading.Thread(target=work, daemon=True).start()
    def stop(self):
        ...
        def work():
            try:
                ...
                Clock.schedule_once(lambda *_: info_popup('Error', f'Stop failed: {e}'))
        threading.Thread(target=work, daemon=True).start()
    def delete(self):
        n = self.spinner.text
        if not n or n.startswith('('):
        ...
        def work():
            try:
                ...
                Clock.schedule_once(lambda *_: info_popup('Error', f'Delete failed: {e}'))
        threading.Thread(target=work, daemon=True).start()
    def set_model(self):
        n = self.spinner.text
        if not n or n.startswith('('):
...
class RemoteModelsPanel(BoxLayout):
    def __init__(self, list_fn, pull_fn, **kw):
        ...
        for label, cb in [('Refresh', self.refresh), ('Pull', self.pull)]:
            b = Button(text=label, size_hint_x=None, width=170, font_size=FONT_SMALL)
            b.bind(on_release=lambda *_, _cb=cb: _cb())
            row.add_widget(b)
        self.add_widget(row)
        out_wrap = ScrollView(size_hint=(1, 1), bar_width=12)
    ...
    def refresh(self):
        def work():
            try:
                ...
                Clock.schedule_once(lambda *_: info_popup('Error', f'Remote list failed: {e}'))
        threading.Thread(target=work, daemon=True).start()
    def _update(self, names):
        self.spinner.values = names
        self.spinner.text = names[0] if names else '(refresh)'
    ...
    def pull(self):
        ...
        def work():
            try:
                ...
                Clock.schedule_once(lambda *_: info_popup('Error', f'Pull failed: {e}'))
        threading.Thread(target=work, daemon=True).start()
    ...
    def refresh_models(self):
        def work():
            try:
                ...
                Clock.schedule_once(lambda *_: info_popup('Error', f'Model list failed: {e}'))
        threading.Thread(target=work, daemon=True).start()
...
class BatLLMConfigApp(App):
    ...
    def build(self):
        root = BoxLayout(orientation='vertical', padding=(PAD_X, PAD_Y), spacing=PAD_Y)
        banner = Label(text='BatLLM Configurator (use with care and make backups!)',
                       color=(.4, 1, .4, 1), size_hint_y=None, height=40,
                       font_size=FONT_BASE, bold=True, halign='left', valign='middle')
        root.add_widget(banner)
        path_row = BoxLayout(size_hint_y=None, height=ROW_HEIGHT, spacing=20)
        path_row.add_widget(Label(text='Config:', size_hint_x=None,
                            width=100, font_size=FONT_SMALL))
        ...
        btn_test = Button(text='Test Conn', size_hint_x=None, width=200, font_size=FONT_SMALL)
        ...
        save_row = BoxLayout(size_hint_y=None, height=ROW_HEIGHT - 4, spacing=1)
        for label, cb in [('Save LLM', self.save_llm), ('Save RAW', self.save_raw_yaml),
                          ('Save Other', self.save_other), ('Undo', self._snap_undo)]:
            b = Button(text=label, size_hint_x=None, width=200, font_size=FONT_SMALL)
            b.background_color = (0.7, 0.75, 0.85, 1) if label != 'Undo' else (0.9, 0.7, 0.7, 1)
            b.bind(on_release=lambda *_, _cb=cb: _cb())
        ...
        self._add_tab(self.tabs, 'LLM', self.llm_panel)
        self.raw_panel = RawYAMLPanel()
        self._add_tab(self.tabs, 'YAML', self.raw_panel)
        self.snap_panel = SnapshotPanel(self._snap_list, self._snap_activate,
                                        self._snap_save, self._snap_undo, self._snapshot_dir)
        self._add_tab(self.tabs, 'Snapshots', self.snap_panel)
        self.other_panel = OtherSettingsPanel()
        self._add_tab(self.tabs, 'Other', self.other_panel)
        ...
        self._add_tab(self.tabs, 'Local Models', self.local_panel)
        self.remote_panel = RemoteModelsPanel(self._remote_models, self._pull_model)
        self._add_tab(self.tabs, 'Remote Models', self.remote_panel)
        self.console_panel = ConsolePanel(self)
        self._add_tab(self.tabs, 'Console', self.console_panel)
        root.add_widget(self.tabs)
        self.status = Label(text='Ready.', size_hint_y=None, height=42,
        ...
        Clock.schedule_once(self._resize_tabs, 0.05)
        return root
    def _resize_tabs(self, *_):
        if not hasattr(self, 'tabs') or not self.tabs.tab_list:
            return
        for btn in self.tabs.tab_list:
            try:
                btn.font_size = FONT_SMALL
                btn.text_size = (None, None)
                btn.texture_update()
                req_w = btn.texture_size[0] + 50
                btn.size_hint_x = None
                btn.width = max(req_w, 140)
            except Exception:
                pass
        self.tabs.do_layout()
    def _ensure_config_file(self, path: str):
        ...
        minimal = {'llm': {'url': 'http://localhost', 'port': 11434, 'model': '',
    ...
    def load_config(self):
        ...
        if not hasattr(self, 'cfg_path') or self.cfg_path is None:
            default_cfg = self._default_config_path()
            self.cfg_path = TextInput(text=default_cfg, multiline=False, font_size=FONT_SMALL)
        path = (self.cfg_path.text or '').strip()
        if not path:
            path = self._default_config_path()
            self.cfg_path.text = path
        try:
