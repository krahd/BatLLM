...
class Bullet:
    ...
    def update(self, bots):
        ...
        if self.x < 0 or self.x > 1 or self.y < 0 or self.y > 1:
            alive = False
            damaged_bot_id = None
            return alive, damaged_bot_id
        
        # Check for collision with bots
        for bb in bots:
            if bb.id != self.parent_id:                
                #if self.segment_hits_bot2(bb, (x, y), (nx, ny)):
                if self.segment_hits_bot2(bb, (x, y), (nx, ny)):
                    alive = False
                    damaged_bot_id = bb.id
                    return alive, damaged_bot_id
    ...
    def segment_hits_bot2(self, bot, p1, p2):
        """
        Check if the segment from p1 to p2 hits the bot's body, 
        considering shield status and coverage.

        Parameters:
            p1: tuple (x1, y1)
            p2: tuple (x2, y2)
            bot: object with attributes:
                - x, y: center of the bot
                - diameter: diameter of the bot (for circle radius)
                - rot: bot rotation in radians
                - shield: bool
                - shield_range: float in degrees

        Returns:
            True if the bot is hit (unshielded hit), False otherwise.
        ...
        b = 2 * (fx * dx + fy * dy)
        c = fx**2 + fy**2 - radius**2 # fx^2 + fy^2 - radius^2

        discriminant = b**2 - 4 * a * c
        # If the discriminant is negative there are no solutions, hence no intersection
        if discriminant < 0:
            return False  
        ...
        if not bot.shield:
            return True  # no shield means hit (TODO move up)

        # Shield logic
        impact_angle = math.atan2(hit_y - cy, hit_x - cx)
        shield_range_rad = math.radians(bot.shield_range)

        # Normalize difference between angles to [-π, π]
        def angle_diff(a, b):
            d = a - b
            return (d + math.pi) % (2 * math.pi) - math.pi

        diff = angle_diff(impact_angle, bot.rot)

        if abs(diff) <= shield_range_rad:
            return False  # hit falls inside the shield
        
        return True  # outside the shield arc thus damage
