...
Generated corpus files and result summaries are committed and regenerated by the dedicated URUCON workflow. That workflow runs on Ubuntu with Python 3.12. It compiles the research-facing source, validates the schema and committed artefact, runs the focused trace, replay, packaging, and Homebrew tests used by the research package, executes every research experiment, and rebuilds the software artefact without duplicating the manuscript.

The broader application test matrix is maintained separately in the main CI workflow on Ubuntu, macOS, and Windows with Python 3.10–3.12.
