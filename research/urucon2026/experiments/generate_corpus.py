"""Generate and verify the deterministic-command evaluation corpus."""
...
from game.trace_contract import PrivacyMode
...
def build_session(
    index: int, plays: int, privacy: PrivacyMode
) -> dict[str, object]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    for index in range(args.sessions):
        payload = build_session(index, args.plays, modes[index % len(modes)])
    ...
    summary = {
        ...
        "privacy_modes": {
            mode.value: sum(
                row["privacy_mode"] == mode.value for row in rows
            )
            for mode in modes
        },
    }
